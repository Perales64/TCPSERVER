    ...
    switch (error_type) {
        ...
        case ERROR_NETWORK:
            ...
            if (is_critical_path && error_stats.network_errors < 5) {
                printf("Intentando recuperación de red...\n");
        ...
        default:
            ...
            printf("ERROR CRÍTICO en %s: 0x%08lX (Contador: %lu)\n",
                   context, error_code, error_stats.critical_errors);
            
            if (is_critical_path && error_stats.critical_errors > 3) {
                printf("DETENCIÓN DEL SISTEMA: Demasiados errores críticos\n");
                CY_ASSERT(0);
            }
            return false;
    }
...
static void client_task(void *param) {
    ...
    printf("[%lu] CLIENTE %lu CONECTADO (ranura %d)\n", 
           client->client_id, client->client_id, client_index);
    ...
    while (client->state == CLIENT_STATE_ACTIVE) {
        ...
        if (result == CY_RSLT_SUCCESS && bytes_received > 0) {
            ...
            if (!handle_error_enhanced("Recepción de cliente", result, false)) {
                printf("Cliente %lu desconectado debido a error\n", client->client_id);
    ...
    printf("Finalizando tarea de cliente para cliente %lu\n", client->client_id);
...
static void accept_new_client(void) {
    ...
    if (result == CY_RSLT_SUCCESS) {
        ...
        if (client_index >= 0) {
            ...
            const char *reject_msg = "Servidor lleno, intente más tarde\n";
            uint32_t bytes_sent;
        ...
        handle_error_enhanced("Aceptar conexión", result, false);
    }
}
...
static cy_rslt_t connect_wifi(void) {
    ...
    while (retry_count < MAX_RETRIES) {
        ...
        if (result == CY_RSLT_SUCCESS) {
            printf("¡WiFi conectado! IP: %lu.%lu.%lu.%lu\n",
        ...
        if (!handle_error_enhanced("Conexión WiFi", result, true)) {
            break;
        }
...
static cy_rslt_t create_server_socket(void) {
    ...
    printf("Servidor TCP mejorado escuchando en puerto %d (máximo %d clientes)\n",
...
static void print_server_status(void) {
    ...
    if ((current_time - last_status_time) > 30000) {
        ...
        printf("Estadísticas de errores - Recup: %lu, Red: %lu, Crít: %lu\n",
...
void tarea_TCPserver(void *arg) {
    ...
    do {
        result = connect_wifi();
        if (result != CY_RSLT_SUCCESS) {
            printf("Conexión WiFi falló, reintentando en %d segundos...\n",
    ...
    if (result != CY_RSLT_SUCCESS) {
        handle_error_enhanced("Inicialización de sockets", result, true);
        vTaskDelete(NULL);
        return;
    }
    ...
    do {
        ...
        if (result != CY_RSLT_SUCCESS) {
            if (!handle_error_enhanced("Creación del servidor", result, true)) {
                vTaskDelay(pdMS_TO_TICKS(SERVER_RECOVERY_DELAY_MS));
            }
        }
    } while (result != CY_RSLT_SUCCESS);
