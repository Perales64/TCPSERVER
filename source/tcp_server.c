...
static void client_task(void* param)
{
    ...
    printf("Client task started for client %lu (slot %d)\n", 
           client->client_id, client_index);
    ...
    while (client->state == CLIENT_STATE_ACTIVE) {
        ...
        if (result == CY_RSLT_SUCCESS && bytes_received > 0) {
            ...
            result = cy_socket_send(client->socket, buffer, bytes_received,
                                   CY_SOCKET_FLAGS_NONE, &bytes_sent);
            
            if (result != CY_RSLT_SUCCESS) {
                if (!handle_error_enhanced("Client send", result, false)) {
                    client->state = CLIENT_STATE_ERROR;
                    break;
                }
            } else {
                printf("Echo sent to client %lu\n", client->client_id);
            }
            
            // Here you could add message processing logic
            // For example, check for specific commands and send to queues
...
static void print_server_status(void)
{
    ...
    if ((current_time - last_status_time) > 30000) {
        ...
        printf("Error stats - Recoverable: %lu, Network: %lu, Critical: %lu\n",
...
void tarea_TCPserver(void *arg)
{
    ...
    printf("=== Enhanced TCP Server Starting ===\n");
    ...
    server_running = true;
    printf("Enhanced TCP server ready!\n\n");
